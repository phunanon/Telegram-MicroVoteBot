import { PDFDocument, StandardFonts, rgb, RGB } from 'https://cdn.skypack.dev/pdf-lib@^1.16.0?dts';
import fontkit from 'https://cdn.skypack.dev/@pdf-lib/fontkit@^1.0.0?dts';
import { LawResult, LawResultStatus } from "./LawResult.ts";
import { n2id } from "./db.ts";

const size = 12, margin = size * 3;
...
export async function makeConstitution (laws: LawResult[], chatName: string): Promise<Uint8Array> {
  ...
  drawText(`Laws of ${chatName}`, 2, rgb(0.25, .75, 0.25));
  const dateText = new Date().toISOString().replace(/(T|\..+)/g, ' ').trim();
  ...
  laws.forEach(({law, status, pc}) => {
    ...
    const statusStr = `(${n2id(law.TimeSec)}) ${status}${[LawResultStatus.Accepted, LawResultStatus.Rejected].includes(status) ? ` at ${pc.toFixed(2)}% approval.` : ""}`;
    drawText(statusStr, 1, rgb(0.5, 0.5, 0.5));

    drawText(law.Body, 1, rgb(0, 0, 0));

    nextY += (size * 1.5) + (numLines(law.Body) * size * 1.2) + (numWrappedLines(law.Body) * size * 1.2);
  });

  return await pdfDoc.save();
}

const numWrappedLines = (str: string) => str.split("\n").map(l => Math.floor(l.length / 100)).reduce((a, b) => a + b);
